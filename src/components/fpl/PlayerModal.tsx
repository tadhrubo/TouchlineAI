...
function getGameweekBreakdown(player: Player): PointEvent[] {
  ...
  if (total <= 0) {
    return [{ name: "Minutes Played", count: "0'", pts: "0 pts" }];
  }
...
export const PlayerModal: React.FC<PlayerModalProps> = ({
  ...
  const pointEvents = getGameweekBreakdown(player);
    ...
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 bg-black/60 backdrop-blur-sm animate-fade-in select-none">
      ...
      <div className="bg-[#0B0E14] border border-white/[0.08] w-full sm:w-[400px] rounded-t-2xl sm:rounded-2xl pb-safe shadow-2xl overflow-hidden animate-slide-up flex flex-col max-h-[85vh]">
        ...
        {player.status !== "available" && player.news && (
          <div className="mx-4 mt-3 p-2.5 rounded-lg bg-neutral-900/60 border border-white/[0.06] text-neutral-300 text-xs flex items-center justify-between">
        ...
        {/* Real GW Points Breakdown Ledger */}
        ...
        <div className="p-3 bg-neutral-950/60 border-t border-white/[0.06] flex flex-col gap-2">
          <button
            onClick={() => {
              onClose();
              onDiscuss(player);
          ...
          <button
            onClick={onClose}
            className="w-full py-2.5 text-sm font-medium text-neutral-400 hover:text-neutral-200 bg-transparent border border-neutral-800 rounded-lg transition-colors text-center"
          >
            Close
          </button>
        </div>
